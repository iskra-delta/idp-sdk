...
sio_port *serial_init_ex(sio_port_addr port_addr, sio_mode mode, sio_bauds bauds,
    ...
    for (i = 1; i < 11; i++) {
        _serial_port_out(_serial_ctrl_port(port_addr),
            _serial_init_value(i, mode, bauds, data_bits, stop_bits, parity));
    }
    ...
    _serial_buffer_alloc_internal(&port->buffer_in, (uint16_t)(in_buffer_sz + in_buffer_ext));
    _serial_buffer_alloc_internal(&port->buffer_out, out_buffer_sz);
    port->in_buffer_ext = in_buffer_ext;
    port->no_activity_thr = no_activity_thr;
    port->addr = port_addr;
    port->wr5 = _serial_init_wr5_default();
